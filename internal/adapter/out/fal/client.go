...
func (c *Client) Submit(ctx context.Context, generationRequest generation.Request) (generation.Job, error) {
	spec, err := (Compiler{}).Compile(PromptInput{
		Direction:       generationRequest.Direction,
		ReferenceImages: generationRequest.References,
	})
	if err != nil {
		return generation.Job{}, err
	}
	if strings.TrimSpace(spec.Prompt) == "" {
		return generation.Job{}, fmt.Errorf("submit fal request: prompt is required")
	}
	request := submitRequest{
		...
		ReferenceVideoURLs:  spec.ReferenceVideoURLs,
		ReferenceAudioURLs:  spec.ReferenceAudioURLs,
	}
...
func (c *Client) requestURL(requestID string) string {
	return c.modelURL() + "/requests/" + url.PathEscape(requestID)
}
