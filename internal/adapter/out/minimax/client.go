...
func (s Settings) Validate() error {
	...
	if s.Duration != 5 {
		return fmt.Errorf("duration must be 5 seconds for the live segment contract")
	}
...
func (c *Client) Submit(ctx context.Context, request generation.Request) (generation.Job, error) {
	settings := c.Settings()
	if len(request.References) > 0 {
		return generation.Job{}, fmt.Errorf("submit MiniMax request: multimodal references are not supported online")
	}
	spec := request.Spec
	if spec.Mode == "" {
	...
	err := c.doJSON(ctx, http.MethodPost, c.baseURL+"/v2/video_generation", createRequest{
		Model: settings.Model, Content: content, Resolution: settings.Resolution,
		Duration: settings.Duration, Ratio: spec.Ratio,
...
func (c *Client) Result(ctx context.Context, taskID string) (generation.Result, error) {
	...
	settings := c.Settings()
	price, err := UnitPriceCNY(settings.Model, settings.Resolution)
	if err != nil {
		return generation.Result{}, err
	}
...
func (c *Client) Cancel(ctx context.Context, taskID string) error {
	...
	endpoint := c.baseURL + "/v2/video_generation/cancel?task_id=" + url.QueryEscape(taskID)
