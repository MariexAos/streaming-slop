...
func (c *Client) Observe(ctx context.Context, input audience.Input) (audience.Observation, error) {
	...
	result, err := c.complete(ctx, settings.baseURL, key, chatRequest{
		...
		ResponseFormat: schemaFormat("observation", audience.ObservationSchema()),
	})
	if err != nil {
		return audience.Observation{}, fmt.Errorf("observe with Qwen: %w", err)
	}
	return audience.Decode([]byte(result))
}
...
func (c *Client) Direct(ctx context.Context, input director.Input) (live.Direction, error) {
	...
	result, err := c.complete(ctx, settings.baseURL, key, chatRequest{
		...
		Messages:       []chatMessage{{Role: "system", Content: director.Instruction}, {Role: "user", Content: string(prompt)}},
		ResponseFormat: schemaFormat("direction", director.Schema()),
	})
	if err != nil {
...
var _ audience.Observer = (*Client)(nil)
