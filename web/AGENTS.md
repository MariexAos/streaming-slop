...
- App 负责页面组装，store 中的 Hook 管理连接和交互状态；components/ui 不依赖业务组件、store 或 API；lib 不依赖 store 或 UI；实际 import 边界以 dependency-cruiser 配置为准。
- Effect 显式声明依赖。Zustand 使用稳定 action 作为连接依赖，不能将变化的整个快照作为重连触发条件；连接、定时器和请求必须清理。
