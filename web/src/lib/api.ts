...
export async function fetchSnapshot(signal?: AbortSignal): Promise<OpsSnapshot> {
  ...
  return parseResponse(response, opsSnapshotSchema.parse)
}
...
export async function sendCommand(command: OpsCommand): Promise<CommandResponse> {
  ...
  const response = await fetch(
    isFallback ? "/api/v1/ops/fallback" : `/api/v1/ops/${command}`,
    {
  ...
  return parseResponse(response, commandResponseSchema.parse)
}
...
export async function fetchGenerationConfig(signal?: AbortSignal): Promise<GenerationConfig> {
  ...
  return parseResponse(response, generationConfigSchema.parse)
}

export async function updateGenerationConfig(config: Pick<GenerationConfig, "model" | "resolution" | "durationSeconds" | "ratio"> & { apiKey?: string }): Promise<GenerationConfig> {
  const response = await fetch("/api/v1/config/generation", {
    method: "PUT",
...
export async function fetchBilibiliConfig(signal?: AbortSignal): Promise<BilibiliConfig> {
  ...
  return parseResponse(response, bilibiliConfigSchema.parse)
}

export async function updateBilibiliConfig(config: { roomId: number; cookie?: string }): Promise<BilibiliConfig> {
  const response = await fetch("/api/v1/config/bilibili", {
    method: "PUT",
    headers: { Accept: "application/json", "Content-Type": "application/json" },
    body: JSON.stringify(config),
...
export async function sendMockDanmaku(message: { username: string; text: string }): Promise<CommandResponse> {
...
export async function fetchQwenConfig(signal?: AbortSignal): Promise<QwenConfig> {
  const response = await fetch("/api/v1/config/qwen", { headers: { Accept: "application/json" }, signal })
  return parseResponse(response, qwenConfigSchema.parse)
}

export async function updateQwenConfig(config: { baseUrl: string; apiKey?: string }): Promise<QwenConfig> {
...
export async function fetchFlowCatalog(signal?: AbortSignal): Promise<FlowCatalog> {
  ...
  return parseResponse(response, flowCatalogSchema.parse)
}

export async function fetchCurrentFlow(signal?: AbortSignal): Promise<CurrentFlow> {
  const response = await fetch("/api/v1/flows/current", { headers: { Accept: "application/json" }, signal })
  return parseResponse(response, currentFlowSchema.parse)
}

export async function fetchSessionHistoryList(signal?: AbortSignal): Promise<SessionHistorySummary[]> {
...
export async function fetchSessionHistory(id: string, signal?: AbortSignal): Promise<SessionHistory> {
  const response = await fetch(`/api/v1/sessions/${encodeURIComponent(id)}`, { headers: { Accept: "application/json" }, signal })
  return parseResponse(response, sessionHistorySchema.parse)
}
...
export function openOpsEvents(handlers: {
  ...
  events.addEventListener("snapshot", (event) => {
    try {
      handlers.onSnapshot(opsSnapshotSchema.parse(JSON.parse((event as MessageEvent).data)))
    } catch {
