...
describe("opsSnapshotSchema", () => {
  ...
  it("rejects unknown statuses", () => {
    expect(
      opsSnapshotSchema.safeParse({
        ...snapshotFixture(),
        session: { ...snapshotFixture().session, status: "unknown" },
      }).success,
    ).toBe(false)
  })
})
...
describe("flow schemas", () => {
  it("accepts the observable director loop contract", () => {
        ...
        flows: [
          {
            ...
            kind: "primary",
            description: "日常互动",
            nodes: [{ id: "audience", type: "audience-window", label: "弹幕窗口" }],
            edges: [],
          },
        ],
      }).flows,
    ...
    expect(
      currentFlowSchema.parse({
        flowId: "host-dialogue",
        status: "running",
        ...
        direction: {
          ...
          summary: "按观众的选择继续聊。",
          horizonSeconds: 60,
          effectiveInSeconds: 45,
          axes: [{ name: "互动性", current: 30, target: 70 }],
        },
        audience: {
          windowSeconds: 20,
          messageCount: 12,
          summary: "观众想看看桌上的东西。",
          intents: [{ label: "展示物品", support: 0.72 }],
        },
        nodes: [
          {
            id: "audience",
        ...
        beats: [
          {
            ...
            anchorFrame: "chat-live-start → chat-live-end",
            status: "planned",
          },
        ],
      }).direction.horizonSeconds,
    ).toBe(60)
  })
})
...
describe("bilibiliConfigSchema", () => {
  it("accepts only the secret-free connection projection", () => {
    ...
    expect(
      bilibiliConfigSchema.safeParse({
        roomId: 123,
        cookieConfigured: true,
        status: "connected",
        lastError: null,
